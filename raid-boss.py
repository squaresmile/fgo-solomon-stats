...
credentials = ServiceAccountCredentials.from_json_keyfile_name(
    base_path + "google-api.json", scope
)

gc = gspread.authorize(credentials)
rows = gc.open_by_key(
    "1V8BhdZ0mT0IA0rX6xf_YNE31L3vo2W5moaM1MTNcuyQ"
...
bosses = [
    boss for boss in list(df["id"].unique()) if not pd.isnull(boss) and boss != ""
]
boss_df = {}
for boss in bosses:
...
# OCR Error
boss_df["andromalius"] = boss_df["andromalius"][
    boss_df["andromalius"]["time captured"] != 1546038899
]

# Need to improve this part: Make the HP list increasing only
for i in range(10):
    for boss in boss_df:
        boss_df[boss] = boss_df[boss][
            boss_df[boss]["hp"] > boss_df[boss]["hp"].shift(1).fillna(0)
        ]  # Increasing HP only
...
for boss in boss_df:
    ...
    if boss == "barbatos":
        ...
        idx = (y < 20) & (y > 0)
    x = x[idx]
    x = pd.to_datetime(x, unit="s") + pd.Timedelta(
        "-08:00:00"
    )  # Change time to PST time
    y = y[idx]
...
all_kps_plot(
    [boss for boss in boss_df if boss != "barbatos"],
    "all_kps_except_barbatos",
    "All KPS except Barbatos",
)
